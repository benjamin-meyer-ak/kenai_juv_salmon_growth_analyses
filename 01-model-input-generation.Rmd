...
# prep fish ages dataframe for later results table generation at end of this code chunk
...
size_dist <- fish_ages1 %>%
  ...
  summarise(mean_wt_g = round(mean(wt_g),1),
            ...
            `Mean Weight (g)` = paste0(mean_wt_g,"±",stdev_wt_g),
            `Mean Fork Length (mm)` = paste0(mean_len_mm,"±",stdev_len_mm),
            n = n()) %>%
  select(-mean_wt_g,-mean_len_mm,-stdev_len_mm,-stdev_wt_g)
...
z <- anti_join(size_dist,anti_size_dist) %>%
