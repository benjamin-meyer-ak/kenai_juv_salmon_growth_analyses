...
Figure 6
```{r}
# read in and prep observed diet prop data
diet_prop <- read.csv("other/outputs/diet_props_tbl2.csv") %>%
  pivot_longer(cols = c("FishEggs", 
...
ggsave("other/documents/Figure images/Figure 6.png", units = "cm", width = 16, height = 8)
...
Figure 8
```{r}
# read in, prep, and summarise full pct chg results
pct_chg <- read.csv("other/outputs/pct_chg_tbl.csv") %>%
  clean_names() %>%
  filter(climate_scenario == "RCP 8.5") %>%
  mutate(percent_change_in_mass = as.numeric(str_remove(percent_change_in_mass,"%"))) %>%
  group_by(watershed,population,simulation_period,food_consumption_scenario) %>%
  summarise(mean_pct = mean(percent_change_in_mass),
            se_pct = std.error(percent_change_in_mass))
...
# order bars
pct_chg$food_consumption_scenario <- factor(pct_chg$food_consumption_scenario, levels = c("-20%","Mean","+20%"))

# facet titles
temp_scen <- c("RCP 6.0" = "Mid-Range\nEmissions (RCP 6.0)",
              "RCP 8.5"  = "Rapid Increase\nEmissions (RCP 8.5)")
...
pct_chg %>%
  ...
  ggplot(aes(watershed,mean_pct,fill = food_consumption_scenario)) +
  geom_col(position = position_dodge()) +
  geom_errorbar(aes(ymin = mean_pct - se_pct, ymax = mean_pct + se_pct),  colour="black", width=.2, size=.8, position=position_dodge(.9)) +
  facet_grid(population ~ simulation_period, labeller = labeller(temp_scen = temp_scen)) +
  ...
  scale_fill_discrete(breaks=c("-20%","Mean","+20%"),
...
ggsave("other/documents/Figure images/Figure 8.jpg",
       width = 9, height = 5)
...
```{r}
# calculate overall mean chg
pct_chg <- read.csv("other/outputs/pct_chg_tbl.csv") %>%
  clean_names() %>%
  mutate(percent_change_in_mass = as.numeric(str_remove(percent_change_in_mass,"%"))) %>%
  summarise(mean_pct = mean(percent_change_in_mass),
            se = std.error(abs(percent_change_in_mass)))
