...
Update 6/9/2022: We are now working to re-shape our data input structures so that they will cooperate with the most recent version of FB4, v1.1.3.

<br>

### Original model results (2018)

Here we will examine the model simulation results based on inputs from the 2015-2016. As of `r Sys.Date()` these model results are based off the erroneous diet inputs that used dry mass for prey items rather than wet mass.
...
# read in observed sizes
fish_size_dir <- "other/outputs/all_fish_age_size.csv"
obs_fish_size <- read.csv(fish_size_dir) %>%
  clean_names() %>%
  select(river,year,sample_event_num,spp,wt_g,len_mm,age,date,
         hydrologic_segment,season,doy) %>%

# summarise data to prep for plot
  group_by(river,year,spp,age,
         hydrologic_segment,doy) %>%
  filter(!is.na(wt_g)) %>%
  summarise(n = n(),
            sd_wt_g = sd(wt_g),
            avg_wt_g = mean(wt_g)) %>%
  ...
  mutate(source = "field_data") %>%
  transform(year = as.factor(year))

# stack dataframe with simulated results
# first, modify column format of simulation outputs
sim_results <- results %>%
  select(year,site,spp,age,wt_g_old_param,doy,scenario) %>%
  separate(site, sep = " ", into = c("a1","b1","c1")) %>%
  rename("hydrologic_segment" = "a1",
         "source" = "scenario",
         "avg_wt_g" = "wt_g_old_param") %>%
  mutate(river = paste(b1,c1)) %>%
  select(river,year,spp,age,hydrologic_segment,
         doy,avg_wt_g,source) %>%
  transform(age = as.integer(age))

obs_sim_results <- bind_rows(obs_fish_size,sim_results)

# plot
obs_sim_results %>%
  mutate(sim_name = paste0(hydrologic_segment,"_",river,"_age_",age,"_",spp),
         cohort = paste0("age_",age,"_",spp),
         site = paste(hydrologic_segment,river)) %>%
  ggplot(aes(doy,avg_wt_g,shape = source)) +
  geom_point() +
  facet_grid(site~year)

  pivot_wider(names_from = "source", values_from = "avg_wt_g") %>%
  arrange(river,hydrologic_segment,year,spp,age,doy)
```
