...
write.csv(diet,dir, row.names = F)
...
How many sampling events had n < 3 fish per cohort? Eliminate these.
```{r}
dat15 <- read_excel(SCTC2015, sheet = "C 2015 Diet & LW Data") %>%
  clean_names() %>%
  select(sample_event_num,sample_event,river,reach_name,spp,age_manual2,weight_g,length_mm) %>%
  transform(weight_g = as.double(weight_g)) %>%
  mutate(year = "2015")
dat16 <- read_excel(SCTC2016, sheet = "C 2016 Diet & LW Data") %>%
  clean_names() %>%
  select(sample_event_num,sample_event,river,reach_name,spp,age_manual2,weight_g,length_mm) %>%
  mutate(year = "2016")

dat <- bind_rows(dat15,dat16) %>%
  rename(age = age_manual2) %>%
  filter(spp %in% c("Coho","Chinook"),
         !is.na(weight_g),
         !is.na(length_mm),
         age != 2) %>%
  group_by(sample_event,sample_event_num,year,river,reach_name,spp,age) %>%
  summarise(n = n()) %>%
  filter(n > 2) %>%
  mutate(season = case_when(
    sample_event_num == 1 ~ "Early Summer",
    sample_event_num == 2 ~ "Mid-Summer",
    sample_event_num == 3 ~ "Late Summer",
    sample_event_num == 4 ~ "Fall"
  )) %>%
  mutate(sim_name = paste0(year,"_",season,"_",reach_name," ",river,"_",spp,"_Age_",age)) 
```
...
# 6/27/22 -- this step results in elimination of cohorts that we want to include
...
```{r, echo = F}
# generate csv of wet mass of each prey item assigned to each fish for use in later scripts
dir <- "other/outputs/fish_prey_wet_mass.csv"
write.csv(diet,dir, row.names = F)
```
