...
invert_function = function(p){
  #p = p[-1,]
  column_names <- c("Simulation",as.vector(p$Parameter))
  column_names
  p1 <- as.data.frame(t(p)) %>%
    rownames_to_column()
  colnames(p1) <- column_names
  p1 = p1[-1,]
  return(p1)
}

run_model_function = function(parms,input_folder,Sp,fit.to,Init_W,Final_W,First_day,
                 Last_day,Ind,Oxycal,
  ...
  #p_value <- Final_W # input$FinW               ### Proporation of Cmax eaten
  ...
  FB4.version = "v1.0.3"
  FB4.File_dir = getwd()
  FB4_Log_File        = "FB4_Log_File.csv"
  #input_folder = 'Main_Inputs'
  Temperature_File = paste0(input_folder,"/Temperature.csv") # Temperature (deg C), over time
  Diet_prop_File   = paste0(input_folder,"//Diet_prop.csv")   # Diet proportions, by prey type, over time
  Prey_E_File      = paste0(input_folder,"/Prey_E.csv")      # Energy density, by prey type, over time
  Indigestible_Prey_File = paste0(input_folder,"/Indigestible_Prey.csv") # Fraction indigestible, by prey type, over time
  Predator_E_File  = paste0(input_folder,"/Pred_E.csv")      # Predator energy density, over time; only read if PREDEDEQ == 1
  #   Mortality
  ...
  p.t <- ifelse(fit.to=="Ration" || fit.to=="Ration_prey",NA,format(round(p,4), nsmall = 3))
  Growth <- format(round(W1.p[(Last_day-First_day+1),4],3), nsmall = 3)
  ...
  W.final  <- W1.p[(Last_day-First_day+1),4]   # final weight (g)
  ED.final <- W1.p[(Last_day-First_day+1),16]  # final energy density (J/g)
  BodyE.final <- W.final*ED.final  # Final total body energy (J) = Final weight (g) * final ED (J/g)
  TotEgain <- W1.p[(Last_day-First_day+1),27]  # total energy gain (J)
  TotSpawnE <- W1.p[(Last_day-First_day+1),23]  # total energy lost in spawning (J)
  ...
  return(list(log = FB4.Log0, summary = FB4.Summary, W1.p = W1.p))
