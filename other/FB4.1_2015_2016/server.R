...
shinyServer(function(input, output,session) {
  choicePrey = reactive({
    ...
    choicePreyJoules <- names_list
    choicePreyJoules
    
    consumptions_of_prey_list = list()
    for(entry in choicePreyJoules){
      consumptions_of_prey_list[[paste('Consumption of Prey (g)', entry)]] = paste("Cons",entry,"g")
      consumptions_of_prey_list[[paste('Consumption of Prey (J)', entry)]] = paste("Cons",entry,"J")
      consumptions_of_prey_list[[paste('"Population Consumption of Prey (g)"', entry)]] = paste("Cons Pop",entry,"g")
      consumptions_of_prey_list[[paste('"Population Consumption of Prey (J)"', entry)]] = paste("Cons Pop",entry,"J")
      
    }
    consumptions_of_prey_list
    choicePreyJoules <- paste("Cons",choicePreyJoules,"J", sep = " ")
    choicePreyJoules
    
    choicePreyGrams <- names_list
    choicePreyGrams <- paste("Cons",choicePreyGrams,"g", sep = " ")
    choicePreyGrams
    choicePreyPopJoules <- names_list ### Allows the consumption output to be broken down by species for the population
    choicePreyPopJoules <- paste("Cons Pop",choicePreyPopJoules,"J", sep = " ")
    choicePreyPopJoules
    choicePreyPopGrams <- names_list
    choicePreyPopGrams <- paste("Cons Pop",choicePreyPopGrams,"g", sep = " ")
    choicePreyPopGrams
    print(choicePreyGrams)
    print(choicePreyJoules)
    print(choicePreyPopGrams)
    print(choicePreyPopJoules)
    #full_column_list
    #full_column_list[["Consumption of Prey (g)"]] = choicePreyGrams
    #full_column_list[["Consumption of Prey (J)"]] = choicePreyJoules
    #full_column_list[["Population Consumption of Prey (g)"]] = choicePreyPopGrams
    #full_column_list[["Population Consumption of Prey (J)"]] = choicePreyPopJoules
    #full_column_list[['Simulation']] = 'Simulation'
    full_column_list = c(full_column_list,consumptions_of_prey_list)
    
  list('consumptions_of_prey_list' = consumptions_of_prey_list,
    'full_column_list' = full_column_list,
    'choicePreyJoules' = choicePreyJoules,
       "choicePreyGrams" = choicePreyGrams,
      'choicePreyPopJoules' = choicePreyPopJoules,
       'choicePreyPopGrams' = choicePreyPopGrams)
  })
  
  output$temp_text = renderText(choicePrey()$choicePreyPopJoules)
  
  # int_df = reactive(values$df)
  # 
  # output$spec_ui = renderUI({
  #   print('spec_ui')
  #   df = int_df()
  #   selectInput("spec",  ### Species selection
  #               label = "Species",
  #               choices = c("",choicesSpecies),
  #               #selected = "Walleye (adult)")
  #               selected = df[df$Simulation == input$Simulation,'Sp'])
  # })
  
  setting_df = reactive({
    print('setting_df')
    df = data.frame('Simulation' = input$Simulation,
    ...
    'fit.to'  = input$fitto,       ### Fit to either final weight, consumption, ration(g), ration(%) or p-value
    
    'Init_W'  = input$InW,                ### Initial weight in g wet weight
    
    'fitting_value'  = input$FinW,              ### Final weight in g wet weight
    ...
    df
     # stored_df = values$df
     # if(df$Simulation %in% stored_df$Simulation){
     #   stored_df = stored_df[-grep(paste0('^',df$Simulation,'$'),stored_df$Simulation),]
     #   stored_df = rbind(stored_df,df)
     #   #stored_df[grep(df$Simulation,stored_df$Simulation),] =  df
     #   values$df = stored_df
     #   isolate(values$df)
     # }else{
     #   isolate(values$df <- rbind(values$df, df))
     # }
    df
  })
  
  values <- reactiveValues(button_1 = 0,button_2 = 0, datafile = 'none')
  
  values$df <- data.frame('Simulation' = character(0),
                          'input_folder' = character(0),
                          'Sp' = character(0),
                          'fit.to'  = character(0),
                          'Init_W'  = numeric(0),
                          'fitting_value'  = numeric(0),
                          #'eaten' = numeric(0),
                          #'Ration_prey' = numeric(0),
                          #'Ration'  = numeric(0),
                          #'p_value' = numeric(0),
                          'First_day'   = numeric(0),
                          'Last_day'    = numeric(0),
                          ...
                          'Oxycal' = numeric(0),
                          'calc.nut' = logical(0),
                          'calc.contaminant'=logical(0),
                          'X_Pred'   = numeric(0),
                          'CONTEQ'   = numeric(0),
                          'calc.pop_mort' = logical(0),
                          'calc.spawn' = logical(0))  
  newEntry <- observe({
    if(input$update > values$button_1) {
      newLine <- isolate(setting_df())
      df = values$df
      if(newLine$Simulation %in% df$Simulation){
           df = df[-grep(paste0('^',newLine$Simulation,'$'),df$Simulation),]
           df = rbind(df,newLine)
      ...
      values$button_1 = values$button_1 + 1
    }
  })
  RemoveEntry <- observeEvent(input$remove,{  
    if(input$remove > values$button_2) {
      newLine <- isolate(setting_df())
      df = values$df
      if(newLine$Simulation %in% df$Simulation){
        df = df[-grep(paste0('^',newLine$Simulation,'$'),df$Simulation),]
      ...
      values$button_2 = values$button_2 + 1
    }
  })
  
  
  
  # setting_append = reactive({
  #   if(is.na(values$settings_df) & dim(setting_df)[1] == 1){
  #     values$settings_df = setting_df()
  #   }else{
  #     if(setting_df()$Simulation %in% values$settings_df$Simulation){
  #       values$settings_df = rbind(values_settings_df,settinds_df)
  #     }
  #   }
  # })
  output$settings = renderTable({
    ...
    df
    col_names = colnames(df)[!colnames(df) %in% remove_cols]
    df[,col_names]
    
    })
  output$settings_app = renderTable({
     print('settings_app')
     df = values$df     
      dim(df)
     #if(!is.na(values$datafile)){
       
       inFile <- input$sim_file
       if (is.null(inFile)){
         df = values$df
       }else{
          if(values$datafile != inFile$datapath){
            df = read.csv(inFile$datapath)
            df = df[,c(2:dim(df)[2])]
            print(dim(df))
            values$df = df
            values$datafile = inFile$datapath
            
            isolate(values$df)
            isolate(values$datafile)
          }
       }
     #}
    df
  ...
  output$downloadData_sim <- downloadHandler(
    filename = function() {
      paste("simulation_settings.csv", sep="")
    },
  ...
  model_table_wide = reactive({
    print('model_table_wide')
    df = Model_2()$model_df
    colnames(df) = c('Parameter','Value',"Simulation")
    #df_w = reshape(df,direction = 'long', idvar = c("FB4.Value",'Simulation'), timevar = c("FB4.Param"))
    #View(df_w)
    #library(tidyr)
    df_w = spread(df, key = 'Simulation', value = c("Value"))
    df_w = df_w[-1,]
    p1 = invert_function(df_w)
    p1
  })
  
  output$Model_table = renderTable({
    print('Model_table')
    df_w = model_table_wide()
    df_w
  })
  
  output$downloadData_model <- downloadHandler(
    filename = function() { paste("Model", '.csv', sep='') },
    content = function(file) {
      #W1.p <- Model()
      #write.csv(data.frame(W1.p[c(seq(1,(input$FD-input$ID+1),input$int)),c(input$var1,input$var2,input$var3,input$var4)]), file)
      write.csv(model_table_wide(),file)
    }
  )
  

  
  output$multi_parameter_table = renderTable({
    print('multi_parameter_table')
    df = Model_2()$parameter_df
    p1 = invert_function(df)
    p1
    
  })
  
  output$downloadData_parameters <- downloadHandler(
    filename = function() { paste("Parameters", '.csv', sep='') },
    content = function(file) {
      #W1.p <- Model()
      #write.csv(data.frame(W1.p[c(seq(1,(input$FD-input$ID+1),input$int)),c(input$var1,input$var2,input$var3,input$var4)]), file)
      write.csv(Model_2()$parameter_df,file)
    }
  )
  
  multi_summary_table_wide = reactive({
    df = Model_2()$model_summary
    #colnames(df) = c('Parameter','Value',"Simulation")
    #df_w = reshape(df,direction = 'long', idvar = c("FB4.Value",'Simulation'), timevar = c("FB4.Param"))
    #View(df_w)
    #library(tidyr)
  ...
  output$multi_summary_table = renderTable({
    print('Model_table')
    df = multi_summary_table_wide()
    p1 = invert_function(df)
    p1
    
  })
  
  output$downloadData_summary <- downloadHandler(
    filename = function() { paste("Summary", '.csv', sep='') },
    content = function(file) {
      #W1.p <- Model()
      #write.csv(data.frame(W1.p[c(seq(1,(input$FD-input$ID+1),input$int)),c(input$var1,input$var2,input$var3,input$var4)]), file)
      write.csv(multi_summary_table_wide(),file)
    }
  )
  
  output$full_output_table = renderDataTable({
    print('full_output_table')
    full_column_list = choicePrey()$full_column_list
    full_column_list
    df = Model_2()$W1.p_l
    new_cols = full_column_list[full_column_list %in% colnames(df)]
    dim(df)
    length(new_cols)
    df_r = df[,paste(new_cols)]
    dim(df_r)
    colnames(df_r) = names(new_cols)
    df_m = cbind(df_r,df[,!colnames(df) %in% paste(new_cols)])
    colnames(df_m)
    re_order = c(colnames(df_m)[length(colnames(df_m))],colnames(df_m)[c(1:length(colnames(df_m))-1)])
    re_order
    df_m[,re_order]
    })  
 ...
 multi_W1.p = reactive({
    ...
    df_r = df[df$Simulation %in% input$Simulation_select,]
    day_range = seq(min(df$Day,na.rm = T),max(df$Day,na.rm=T),input$int)
    df_d = df_r[df_r$Day %in% day_range,]
    
    #df_n = df_d[,c(input$var1,input$var2,input$var3,input$var4,'Simulation')]
    df_n = df_d[,c(input$select_variable_columns,input$select_population_columns,input$select_nutrient_columns,input$select_contaminant_columns,input$select_consumption_columns,'Simulation')]
    df_n
    
    
  })
  
  output$multi_W1.p = renderDataTable({
    print('multi_W1.p')
    df = multi_W1.p()
    full_column_list = choicePrey()$full_column_list
    full_column_list
    ...
    dim(df_r)
    colnames(df_r) = names(new_cols)
    add_df = data.frame(df[,colnames(df)[!colnames(df) %in% paste(new_cols)]])
    add_df
    colnames(df)[!colnames(df) %in% paste(new_cols)]
    colnames(add_df) = colnames(df)[!colnames(df) %in% paste(new_cols)]
    add_df
    df_m = cbind(df_r,add_df)
    re_order = c(colnames(df_m)[length(colnames(df_m))],colnames(df_m)[c(1:length(colnames(df_m))-1)])
    re_order
    df_m[,re_order]
    #df_m
  ...
  output$Simulation_ui = renderUI({
    sim_list = unique(Model_2()$W1.p_l$Simulation)
    sim_list = sim_list[order(sim_list)]
    selectInput('Simulation_select','Simulation',sim_list,selected = sim_list, multiple = T)
  })
  
  
  output$failed_model_text = renderTable({
    print('failed_model')
    failed_model = Model_2()$failed_model
    failed_df = data.frame('Simulation Name' = failed_model)
    #if(length(failed_model) > 0){
    #  paste(paste(failed_model,collapse =', '))
    #}
    failed_df
  ...
  output$failed_ui = renderUI({
    print('failed_model')
    failed_model = Model_2()$failed_model
    if(length(failed_model) > 0){
      tabPanel('Failed Model', textOutput('failed_model_text'))
    }
  })
  
  Model_2 = reactive({
    model_df = data.frame(NULL)
    parameter_df = data.frame(NULL)
    model_summary = data.frame(NULL)
    W1.p_l = data.frame(NULL)
    failed_model = c()
    ...
    for(i in c(1:dim(df)[1])){
      for(j in c(1:dim(df)[2])){
        if(is.factor(df[i,j])){
          cmd = paste0(variable_names[j]," = '",df[i,j],"'")
        }else{
          cmd = paste0(variable_names[j],' = ',df[i,j])
        }
        print(cmd)
      ...
      print(input_folder)
      print('run_model')
      model_list = tryCatch({run_model_function(parms,input_folder,Sp,fit.to,Init_W,fitting_value,First_day,
                         Last_day,Ind,Oxycal,
                         calc.nut,calc.contaminant,X_Pred,CONTEQ,
                         calc.pop_mort,calc.spawn)}, 
                         error = function(e) NA)
      #print(model_list)
      if(!is.na(model_list)){
        model_df_n = model_list$log
        model_summary_n = model_list$summary
        model_summary_n$Simulation = Simulation
        #colnames(model_summary_n) = c('Parameter',Simulation)
        W1.p_n = model_list$W1.p
        W1.p_nl = W1.p_n
        W1.p_nl$Simulation = Simulation
        #colnames(W1.p_n) = paste(Simulation,colnames(W1.p_n),collapse = '..')
        model_df_n$Simulation = Simulation
        if(dim(model_df)[1] == 0){
          model_df = model_df_n
          model_summary = model_summary_n
          W1.p = W1.p_n
          W1.p_l = W1.p_nl
  
          #colnames(model_summary) = c('Parameter',Simulation)
        }else{
          model_df = rbind(model_df, model_df_n)
          col_names = c(colnames(model_summary),Simulation)
          model_summary = rbind(model_summary,model_summary_n)
          #colnames(model_summary) = col_names
          #W1.p = cbind(W1.p,W1.p_n)
          col_union = union(colnames(W1.p_l),colnames(W1.p_nl))
          col_union
          col_diff = setdiff(col_union,colnames(W1.p_l))
          col_diff
          W1.p_l[,col_diff] = NA
          W1.p_l = W1.p_l[,col_union]
          col_diff = setdiff(col_union,colnames(W1.p_nl))
          col_diff
          W1.p_nl[,col_diff] = NA
          W1.p_nl = W1.p_nl[,col_union]
          W1.p_l = rbind(W1.p_l,W1.p_nl)
          
      
          
        }
        
        rownames(parms) = parms$Species
        ...
        if(dim(parameter_df)[1] == 0){
          parameter_df = data.frame('Parameter' = Parameter,value = Value)
          colnames(parameter_df) = c('Parameter',Simulation)
        }else{
          parameter_df_n = data.frame(value = Value)
          colnames(parameter_df_n) = Simulation
          parameter_df = cbind(parameter_df,parameter_df_n)
        }
        
      }else{
        failed_model = c(failed_model,Simulation)
      }
    }
    list(model_df = model_df, parameter_df = parameter_df, model_summary = model_summary,W1.p_l = W1.p_l,failed_model = failed_model)
...
output$downloadData <- downloadHandler(
  ...
  content = function(file) {
    #W1.p <- Model()
    #write.csv(data.frame(W1.p[c(seq(1,(input$FD-input$ID+1),input$int)),c(input$var1,input$var2,input$var3,input$var4)]), file)
    write.csv(multi_W1.p(),file)
    }
)

output$variable_column_ui = renderUI({
  print('select_column_ui')
  columns = colnames(Model_2()$W1.p_l)
  #variable_column_list[["Consumption of Prey (g)"]] = choicePrey()$choicePreyGrams
  #variable_column_list[["Consumption of Prey (J)"]] = choicePrey()$choicePreyJoules
  choices = variable_column_list[variable_column_list %in% columns]
  choices
  #choices[["Consumption of Prey (g)"]] = choicePrey()$choicePreyGrams
  #choices[["Consumption of Prey (J)"]] = choicePrey()$choicePreyJoules
  selectInput("select_variable_columns", ### x-axis variables for plot output
              label = "Individual Variables (click in box for options)",
              choices = choices,
              selected = choices[c(1:3)],
              multiple=TRUE)
})

output$population_column_ui = renderUI({
  print('population_column_ui')
  columns = colnames(Model_2()$W1.p_l)
  #population_column_list[["Population Consumption of Prey (g)"]] = choicePrey()$choicePreyPopGrams
  #population_column_list[["Population Consumption of Prey (J)"]] = choicePrey()$choicePreyPopJoules
  
  #"Population Consumption of Prey (g)"=choicePreyPopGrams,
  #"Population Consumption of Prey (J)"=choicePreyPopJoules,
  choices = population_column_list[population_column_list %in% columns]
  #choices[["Population Consumption of Prey (g)"]] = choicePrey()$choicePreyPopGrams
  #choices[["Population Consumption of Prey (J)"]] = choicePrey()$choicePreyPopJoules
  selectInput("select_population_columns", ### x-axis variables for plot output
              label = "Population Variables",
              choices = choices,
              multiple=TRUE)
})

output$nutrient_column_ui = renderUI({
  print('nutrient_column_ui')
  columns = colnames(Model_2()$W1.p_l)
  choices = nutrient_column_list[nutrient_column_list %in% columns]
  selectInput("select_nutrient_columns", ### x-axis variables for plot output
              label = "Nutrient Variables",
              choices = choices,
              multiple=TRUE)
})

output$contaminant_column_ui = renderUI({
  print('contaminant_column_ui')
  columns = colnames(Model_2()$W1.p_l)
  choices = contaminant_column_list[contaminant_column_list %in% columns]
  selectInput("select_contaminant_columns", ### x-axis variables for plot output
              label = "Contaminant Variables",
              choices = choices,
              multiple=TRUE)
})

output$consumption_column_ui = renderUI({
  print('consumption_column_ui')
  consumptions_of_prey_list = choicePrey()$consumptions_of_prey_list
  columns = colnames(Model_2()$W1.p_l)
  choices = consumptions_of_prey_list[consumptions_of_prey_list %in% columns]
  selectInput("select_consumption_columns", ### x-axis variables for plot output
              label = "Consumption Variables",
              choices = choices,
              multiple=TRUE)
})
...
output$xaxis_ui = renderUI({
  ...
  df = multi_W1.p()
  full_column_list = choicePrey()$full_column_list
  choices = full_column_list[full_column_list %in% colnames(df)]
...
output$yaxis_ui = renderUI({
  ...
  full_column_list = choicePrey()$full_column_list
  choices = full_column_list[full_column_list %in% colnames(df)]
  selectInput("yaxis", ### x-axis variables for plot output
              label = "y Axis",
              choices = choices,
              selected = choices[2], multiple=FALSE)
})
output$yaxis2_ui = renderUI({
  df = multi_W1.p()
  full_column_list = choicePrey()$full_column_list
  choices = full_column_list[full_column_list %in% colnames(df)]
  choices[['None']] = 'None'
  selectInput("yaxis2", ### x-axis variables for plot output
              label = "y Axis 2",
              choices = choices,
...
output$g_plot <- renderPlot({
  ...
  if(input$yaxis2 != 'None'){
    data[,input$yaxis2] = data[,input$yaxis2]/input$scale_y2
  }
  #op <- par(mar = c(5,4,4,4) + 0.1)
  p = ggplot(data, aes_string(x = input$xaxis, col = 'Simulation')) +
    geom_line(aes_string( y = input$yaxis), linetype = 1)
  if(input$yaxis2 != 'None'){
    p = p + geom_line(aes_string( y = input$yaxis2), linetype = 2) +
    scale_y_continuous(sec.axis = sec_axis(~.* input$scale_y2 , name = names(full_column_list[full_column_list == input$yaxis2])))
  }
  #cmd = paste0("p = p + scale_linetype_manual('Variable',values=c('",input$yaxis,"'= 1, '",input$yaxis2,"'=2))")
  #print(cmd)
  #try(eval(parse(text = cmd)))
  p = p + theme_bw()
  p = p + xlab(names(full_column_list[full_column_list == input$xaxis]))
  p = p + ylab(names(full_column_list[full_column_list == input$yaxis]))
  
  
  if(input$show_legend == F){
    p = p + theme(legend.position='none')
  }
  print(p)
               

    #scale_linetype_manual("Variable",values=c(paste(input$yaxis)=1,paste(input$yaxis2)=2))
...
output$y_scale_ui = renderUI({
  if(input$yaxis2 != 'None'){
    numericInput('scale_y2','Scale Y Axis',value = 1,step = 0.1)
  }
})
